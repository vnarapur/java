package corejava;
		...
		import java.util.Map;class ListItems {
		// key will act as an id of the value
		private Integer key;
		// value will be the value of the above key
		private String value;// create constructor for reference
		...
		list.add(new ListItems(1, "v"));
		list.add(new ListItems(2, "Love"));
		list.add(new ListItems(3, "s"));
		list.add(new ListItems(4, "For"));
		list.add(new ListItems(5, "Ever"));
		// Map which will store the list items
		Map<Integer, String> map = new HashMap<>();list.forEach(
		(n) -> { map.put(n.getkey(), n.getvalue()); });// Printing the given map
